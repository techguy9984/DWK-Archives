...
import com.cpjd.stayfrosty.boss.JohnCena;
...
public class Lv3_Boss extends GameState {
	...
	private ArrayList<Enemy> cenas;
	
	Minimap m;
	
	public boolean gameOver;
	
	public Lv3_Boss(GameStateManager gsm, Load load, Save save) {
		super(gsm, load, save);
	...
	public void init() {
		...
		tileMap.loadTiledMap("/Maps/Lv3_Boss.txt");
		...
		m = new Minimap("/Minimaps/Lv3_Boss.png",1,1);
	...
	private void populateEnemies() {
		cenas = new ArrayList<Enemy>();
		
		JohnCena cena = new JohnCena(tileMap, player);
		cena.setPosition(654, 800);
		cenas.add(cena);
	...
	private void checkBullets() {
		for(int i = 0; i < cenas.size(); i++) {
			for(int j = 0; j < cenas.get(i).getBulletArray().size(); j++) {
				if(cenas.get(i).getBulletArray().get(j).getCollisionBox().intersects(player.getCollisionBox())) {
					if(!cenas.get(i).isDead()) player.hit(cenas.get(i).getDamage());
				}
			}
		}
	}
	...
	private void checkAttack() {
		player.checkAttack(cenas);
	}
	
	private void checkMelee() {
		for(int i = 0; i < cenas.size(); i++) {
			if(cenas.get(i).getCollisionBox().intersects(player.getCollisionBox())) {
				player.hit(cenas.get(i).getDamage());
			}
		}
	}
