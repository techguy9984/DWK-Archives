...
import com.cpjd.stayfrosty.boss.Gaben;
...
// APPROVED
public class Lv2_Boss extends GameState {
	...
	private ArrayList<Enemy> gabens;
	
	public Lv2_Boss(GameStateManager gsm, Load load, Save save) {
		super(gsm, load, save);
	...
	public void init() {
		...
		tileMap.loadTiledMap("/Maps/Lv2_Boss.txt");
		...
		m = new Minimap("/Minimaps/Lv2_Boss.png",1,1);
	}
	private void save() {
	...
	private void populateEnemies() {
		gabens = new ArrayList<Enemy>();
		
		Gaben gaben = new Gaben(tileMap, player);
		gaben.setPosition(962, 782);
		gabens.add(gaben);
	}
	
	public void update() {
		// Check for completed level
		if(gabens.get(0).isDead()) {
			gsm.setState(GameStateManager.L3_1);
		}
	...
	private void checkAttack() {
		player.checkAttack(gabens);
	}
	
	private void checkMelee() {
		for(int i = 0; i < gabens.size(); i++) {
			if(gabens.get(i).getCollisionBox().intersects(player.getCollisionBox())) {
				player.hit(gabens.get(i).getDamage());
			}
		}
	}
